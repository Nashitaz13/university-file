...
main (int argc, char * argv[] ) {
	...
	while (!Done) {		/* Main Command Loop. */
		printf_s ("\t\t\tServer is awaiting next request.\n");
		if (!ConnectNamedPipe (hNamedPipe, NULL) ||
		   !ReadFile (hNamedPipe, &inb, BUF_SIZ, &nXfer, NULL)) {
		...
		if (strcmp( inb, QUIT ) == 0) {
			Done = 1;
			printf_s ("\t\t\tServer is quitting now.\n");
			strcpy_s(outb, "", "");
		} else {
			fgets (outb, BUF_SIZ, stdin);
		}
		WriteFile (hNamedPipe, &outb,
		 	(strlen (outb) + 1),&nXfer, NULL);
