if(typeof(COMSCORE)=="undefined"){var COMSCORE={};}if(typeof COMSCORE.SiteRecruit=="undefined"){COMSCORE.SiteRecruit={version:"5.1.3",testUrl:"broker-test.js",configUrl:"broker-config.js",builderUrl:"builder.js",allowScriptCaching:false,CONSTANTS:{COOKIE_TYPE:{ALREADY_ASKED:1,DD_IN_PROGRESS:2},STATE_NAME:{IDLE:"IDLE",DDINPROGRESS:"DDINPROGRESS"}}};
COMSCORE.SiteRecruit.Utils=(function(){var A=COMSCORE.SiteRecruit;return{location:document.location.toString(),referrer:document.referrer.toString(),loadScript:function(D,C){if(C&&!A.allowScriptCaching){D=A.Utils.appendQueryParams(D,(new Date()).getTime());
...
},find:function(L){var N=window.location.toString();if(/cs\_tstMode\=(.+)/i.test(N)){if(/([^\#+]*)/.test(N)){N=RegExp.$1;
}}console.log("ACTUAL URL IS "+window.location.toString());console.log("CLEANED URL IS "+N);
var O=0,E;var K=L;var I=[];var M=false;B=0;for(var F=0;K&&F<K.length;F++){var G=false;
var J=K[F];if(J){var D=new RegExp(J.m,"i");if(N.search(D)!=-1){var H=K[F].prereqs;
G=true;if(H){if(!this.isMatchContent(H.content)){G=false;}if(!this.isMatchCookie(H.cookie)){G=false;
}if(!this.isMatchLanguage(H.language)){G=false;}}}if(G){if(J.halt){M=true;break;}else{I.push(J);
B=J.f;}}}}if(M==true){I=null;B=0;return null;}return this.choosePriority(I);},choosePriority:function(D){var F=null;
...
var I=true;for(var G=0;G<M.length;G++){var E=new RegExp(D.elementValue,"i");if(E){if(E.test(M[G].innerHTML)){if(I){I=false;
...
C.executingPath=A.getExecutingPath("broker.js");if(/cs\_tstMode/i.test(window.location)){console.log("FOUND cs_tstMode in URL - loading broker-test.js");
A.loadScript("https://www.microsoft.com/library/svy/"+C.testUrl,true);}else{console.log("DIDN'T FIND TEST MODE IN URL");
}if(C.browser.ie||C.browser.mozilla){A.loadScript(C.executingPath+C.configUrl,true);
}else{return;}},start:function(){this.init();},run:function(){this.config.Events.beforeRecruit();
if(this.config.mobile){var K=new RegExp(this.config.mobile.match,"i");var H=new RegExp(this.config.mobile.kmatch,"i");
if(this.config.mobile.halt==true&&(K.test(navigator.userAgent)||H.test(navigator.userAgent))){return;
}}if(this.config.objStoreElemName){if(C.browser.ie){COMSCORE.SiteRecruit.Utils.UserPersistence.initialize();
